# Create piano sound example code

# @title Create piano sound example code
import random
...
# @title Shape of you 12 first notes
from midiutil import MIDIFile

# Shape of You
degrees = [61, 64, 61, 61, 64, 61, 61, 64, 61, 63, 61, 59]
durations = [3, 3, 2, 3, 3, 2, 3, 3, 2, 3, 3, 2, 3, 3, 2]
track = 0
channel = 0
time = 0  # In beats
tempo = 192
...
instrument = 54  # 0 - piano, 24 - ...
# MyMIDI.addProgramChange(track, channel, time, instrument)
time = 0
for pitch, duration in zip(degrees, durations):
    MyMIDI.addProgramChange(track, channel, time, instrument)
    MyMIDI.addNote(track, channel, pitch, time, 1, volume)
    time += duration / 2
with open("shape_of_you.mid", "wb") as output_file:
